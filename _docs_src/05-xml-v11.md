...
- `ExtraFields`, serialized from the typed `ExtraFieldData` schema as custom extension XML blocks.

`ExtraFields` is reserved for custom extension data accepted by the official XSD `x:any` slot. Known official XML elements such as `TaxId`, `Address`, `Payments`, `References`, `RentReceipt`, `TransportRoute`, `Totals`, and `Transmission` are rejected in `ExtraFields`; use the matching first-class value object instead.
